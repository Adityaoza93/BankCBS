...
public class Transfer extends JFrame {
	...
	public Transfer() {
		...
		setTitle("Transfer - FinVerse PSB");
		setType(Type.POPUP);
		...
		JLabel lblNewLabel_5 = new JLabel("Payee Account");
		...
		JLabel lblNewLabel_8 = new JLabel("Transfer Amount");
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String debit_amount = textField_5.getText();
				int updated_balance  = Integer.parseInt(current_balance) - Integer.parseInt(debit_amount);
				if(Integer.parseInt(current_balance) > Integer.parseInt(debit_amount)) {
				JOptionPane.showMessageDialog(null, "Verify Debit Amount: "+textField_5.getText()+"");
				...
				try {
					...
					Statement stmtmain=con.createStatement();
					String sqlmain="SELECT * from acc_details WHERE acc_no='"+textField_2.getText()+"'";
					stmt_up.execute(sql_b_up);
					ResultSet sqlmain_result=stmtmain.executeQuery(sqlmain);
					while(sqlmain_result.next()) {
						String balancepayee = sqlmain_result.getString("balance"); 
						int totalbalance = Integer.parseInt(balancepayee) + Integer.parseInt(debit_amount);
						String sqlmnt="UPDATE acc_details SET balance = '"+totalbalance+"' WHERE acc_no = '"+textField_2.getText()+"'";
						Statement stmtmaint=con.createStatement();
						stmtmaint.execute(sqlmnt);
					}
					...
					try {
						...
						email.setContent("<h1>Amount of Rs "+debit_amount+" successfully transferred to account "+textField_2.getText()+". Available Balance: Rs "+textField_7.getText()+"</h1>", "text/html");
